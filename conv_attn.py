...
class Conv_Attn(nn.Module):
    def __init__(self, input_dim, n_heads, hidden, num_layers, transformer_dim=16):
        ...
        self.gelu = nn.GELU()
        self.gate = nn.Linear(transformer_dim * LENGTH + transformer_dim * input_dim, 2)
        self.output_linear = nn.Linear(transformer_dim * LENGTH + transformer_dim * input_dim, 1)
    ...
    def forward(self, x):
        ...
        temporal_attn = temporal_attn.reshape(temporal_attn.shape[0], -1)
        channel_attn = channel_attn.reshape(channel_attn.shape[0], -1)
